...
with DAG(
    ...
    task1 = PythonOperator(
        task_id='getToken_morning',
        python_callable=T_GetToken,
        do_xcom_push=True
    )

    task2 = PythonOperator(
        task_id='downloadFiles_morning',
        python_callable=T_DownloadFile,
        op_kwargs={'token': '{{ ti.xcom_pull(task_ids="getToken_morning") }}', 'fileType': fileType},
        on_failure_callback=notify_teams,
    )

    task3 = PythonOperator(
        task_id='pg_upsert_morning',
        python_callable=T_postgres_upsert_dataframe,
        op_kwargs={'fileName': '{{ ti.xcom_pull(task_ids="downloadFiles_morning") }}'},
        on_failure_callback=notify_teams,
    )
